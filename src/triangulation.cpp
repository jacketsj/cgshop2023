...
pair<Vertex_handle, Vertex_handle> Triangulator::edge_vertices(CDT::Edge e) {
	return make_pair(e.first->vertex((e.second + 1) % 3),
									 e.first->vertex((e.second + 2) % 3));
}

bool Triangulator::constrained(CDT::Edge e) {
	return constrained_edges.count(edge_vertices(e)) > 0;
}

void Triangulator::mark_domains(CDT& ct, Face_handle start, int index,
...
void Triangulator::mark_domains(CDT& cdt) {
...
std::vector<SimplePolygon> getTriangles(const Polygon& polygon_with_holes) {
	...
	for (const auto& vert : polygon_with_holes.outer_boundary())
	...
	for (auto& boundary : boundaries) {
		for (size_t i = 0; i < boundary.size(); ++i) {
			...
			triangulator.constrained_edges.emplace(boundary[i], boundary[j]);
			triangulator.constrained_edges.emplace(boundary[j], boundary[i]);
		}
	}
	triangulator.mark_domains(cdt);
...
getTriangles(const std::vector<Polygon>& polygons_with_holes) {
	...
	for (const auto& polygon_with_holes : polygons_with_holes)
		for (const auto& tri : getTriangles(polygon_with_holes))
